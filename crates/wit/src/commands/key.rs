...
impl KeyNewCommand {
    /// Executes the command.
    pub async fn exec(self, terminal: &Terminal, mut config: Config) -> Result<(), CommandError> {
...
impl KeyDeleteCommand {
    /// Executes the command.
    pub async fn exec(self, terminal: &Terminal, config: Config) -> Result<(), CommandError> {
