...
#[derive(Default)]
struct UseTrieNode {
    // Map of segment index to trie node
    children: HashMap<usize, UseTrieNode>,
    // Set of type indexes that are used at this node
    tys: HashSet<usize>,
}

impl UseTrieNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>, trie: &UseTrie) -> fmt::Result {
        ...
        for (i, (segment, child)) in self.children.iter().enumerate() {
            ...
            write!(f, "{segment}::{{", segment = trie.segments[*segment])?;
            child.fmt(f, trie)?;
            write!(f, "}}")?;
        }
        ...
        for (i, type_index) in self.tys.iter().enumerate() {
            if i > 0 {
                write!(f, ", ")?;
            }

            write!(f, "{ty}", ty = trie.types[*type_index])?;
        }
...
impl UseTrie {
    ...
    fn get<'a>(&self, path: impl Iterator<Item = &'a str>) -> Option<impl Iterator<Item = &str>> {
        ...
        for segment in path {
            let (index, _) = self.segments.get_full(segment)?;
            node = node.children.get(&index)?;
        }

        Some(node.tys.iter().map(|i| self.types[*i].as_str()))
    }
        ...
        I::IntoIter: Clone,
    {
        ...
        for segment in path {
            ...
            node = node.children.entry(segment_index).or_default();
        }

        node.tys.insert(type_index);
...
impl fmt::Display for UseTrie {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (segment_index, child) in &self.root.children {
            write!(
                f,
                "use {segment}::{{",
