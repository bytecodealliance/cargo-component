...
impl GitMetadata {
    ...
    pub fn from_package(package: &Package) -> Result<Option<Self>> {
        ...
        let repository = match Repository::discover(package.manifest_path.clone()) {
            ...
            Err(ref e) if e.class() == ErrorClass::Repository && e.code() == ErrorCode::NotFound => return Ok(None),
            Err(e) => Err(e)
        }?;

        let head = match repository.head() {
            Ok(head) => Ok(head),
            Err(ref e) if e.class() == ErrorClass::Reference && e.code() == ErrorCode::UnbornBranch => return Ok(None),
            Err(error) => Err(error)
        }?;
